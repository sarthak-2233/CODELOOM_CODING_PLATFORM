...
const GraphBFS = () => {
  ...
  const buildTreeStepByStep = useCallback(async () => {
    ...
    setLastAction('BST construction complete! Ready for BFS traversal');
    setIsBuilding(false);
  }, [arrayValues, canvasWidth, insertIntoBST]);

  // BFS Traversal (Level Order)
  const bfsTraversalAnimated = useCallback(async () => {
    if (!root) {
    ...
    setLastAction(`Starting BFS (Level Order) Traversal...`);
    ...
    while (queue.length > 0 && !stopTraversalRef.current) {
      while (pauseTraversalRef.current && !stopTraversalRef.current) {
        await new Promise(resolve => setTimeout(resolve, 100));
      }
      if (stopTraversalRef.current) break;
      
      const { node, level } = queue.shift();
      
      setCurrentLevel(level);
      setQueueState(queue.map(q => q.node.value));
      
      result.push(node.value);
      setTraversalResult([...result]);
      setCurrentStep(step++);
      setLastAction(`Visited node: ${node.value} at level ${level}`);
      ...
      if (node.left) {
        ...
        queueDisplay.push(node.left.value);
      }
      if (node.right) {
        queue.push({ node: node.right, level: level + 1 });
        queueDisplay.push(node.right.value);
      }
    ...
    if (!stopTraversalRef.current) {
      setLastAction('BFS (Level Order) Traversal Complete!');
      setIsTraversalPlaying(false);
  ...
  const buildTreeInstant = useCallback(() => {
    ...
    setQueueState([]);
    setCurrentLevel(0);
    setLastAction('Tree built instantly! Click Start BFS Traversal.');
  ...
  // Draw tree on canvas
  const drawTree = useCallback((ctx, node, level = 0) => {
    if (!node) return;
    ...
    if (node.left) {
      ...
      ctx.lineWidth = 2;
      ctx.stroke();
      drawTree(ctx, node.left, level + 1);
    }
    ...
    if (node.right) {
      ...
      ctx.lineWidth = 2;
      ctx.stroke();
      drawTree(ctx, node.right, level + 1);
    }
    ...
    } else if (isCurrentLevel && isTraversalPlaying) {
    ...
    ctx.strokeStyle = isHighlighted ? '#b5fe00' : 'rgba(182, 255, 0, 0.5)';
    ctx.lineWidth = 2;
    ctx.stroke();
    ...
    ctx.shadowBlur = 0;
  }, [highlightedNodeId, traversalResult, isTraversalPlaying, currentLevel, nodeRadius]);
  ...
  useEffect(() => {
    ...
    ctx.strokeStyle = 'rgba(182, 255, 0, 0.05)';
    ctx.lineWidth = 1;
  ...
  useEffect(() => {
    ...
    if (root) {
      ...
      const drawPopupTree = (node) => {
        ...
        ctx.strokeStyle = isHighlighted ? '#b5fe00' : 'rgba(182, 255, 0, 0.6)';
        ctx.lineWidth = 2.5;
        ctx.stroke();
  ...
  return (
    <div className="p-6 text-white h-full overflow-y-auto">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-2" style={{ color: '#B6FF00', fontFamily: 'Space Grotesk' }}>
          Binary Tree - BFS (Level Order) Traversal
        </h1>
        <p className="text-sm" style={{ color: '#E8FFF8', opacity: 0.7 }}>
          Level by level traversal using Queue (Breadth-First Search)
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          {/* Array Input Section */}
          <div className="p-4 rounded-lg" style={{ backgroundColor: 'rgba(182, 255, 0, 0.05)', border: '1px solid rgba(182, 255, 0, 0.2)' }}>
            <h3 className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#B6FF00' }}>
              Array Input
            </h3>
            <input
              type="text"
              ...
              className="w-full bg-[#1a211e] border border-[rgba(182,255,0,0.3)] rounded-lg px-3 py-2 text-sm text-white placeholder:text-gray-500 focus:outline-none focus:border-[#B6FF00] transition-all mb-2"
            />
            <div className="flex gap-2 mb-2">
              <button
                onClick={applyCustomArray}
                className="flex-1 px-3 py-1.5 rounded-lg bg-[#B6FF00] text-[#476700] text-xs font-bold hover:scale-105 transition-all"
              >
                Apply
              </button>
              <button
                onClick={useDefaultArray}
                className="flex-1 px-3 py-1.5 rounded-lg border border-[rgba(182,255,0,0.3)] text-xs hover:border-[#B6FF00] hover:text-[#B6FF00] transition-all"
              >
                Default
              </button>
            </div>
            <button
              ...
              className="w-full px-3 py-1.5 rounded-lg bg-[#68fcbf]/20 text-[#68fcbf] text-xs font-bold hover:bg-[#68fcbf]/30 hover:scale-105 transition-all"
            >
              🎲 Generate Random
            </button>
          </div>
          ...
          <div className="p-4 rounded-lg" style={{ backgroundColor: 'rgba(182, 255, 0, 0.05)', border: '1px solid rgba(182, 255, 0, 0.2)' }}>
            ...
            <div className="space-y-2">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-[#B6FF00] text-[#476700] text-sm font-bold hover:scale-105 transition-all disabled:opacity-50"
              >
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-[#1a211e] border border-[rgba(182,255,0,0.3)] text-sm hover:border-[#B6FF00] transition-all disabled:opacity-50"
              >
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                ...
                <input
                  ...
                  className="flex-1"
                />
                <span className="text-xs text-[#B6FF00] font-mono">{traversalSpeed}ms</span>
              </div>
              ...
              <div className="flex gap-2">
                {!isTraversalPlaying && !isPaused ? (
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-[#B6FF00] text-[#476700] text-sm font-bold hover:scale-105 transition-all disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-[#B6FF00] text-[#476700] text-sm font-bold hover:scale-105 transition-all"
                  >
          ...
          {queueState.length > 0 && (
            <div className="p-4 rounded-lg" style={{ backgroundColor: 'rgba(104, 252, 191, 0.05)', border: '1px solid rgba(104, 252, 191, 0.2)' }}>
              <h3 className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#68fcbf' }}>
                Queue Status
              </h3>
              <div className="flex gap-1 flex-wrap">
                {queueState.map((val, idx) => (
                  <div key={idx} className="px-2 py-1 rounded text-xs bg-[#68fcbf]/20 text-[#68fcbf]">
                    {val}
                  </div>
                ))}
              </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-[#B6FF00]/10 border-2 border-[#B6FF00] text-[#B6FF00] text-sm font-bold hover:bg-[#B6FF00]/20 transition-all"
          >
        ...
        <div className="lg:col-span-3">
          <div className="rounded-lg overflow-hidden" style={{ backgroundColor: 'rgba(182, 255, 0, 0.03)', border: '1px solid rgba(182, 255, 0, 0.2)' }}>
            <div className="p-3 border-b border-[rgba(182,255,0,0.1)]">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#B6FF00' }}>Tree Visualization</span>
          ...
          {/* Traversal Result */}
          <div className="mt-4 p-4 rounded-lg" style={{ backgroundColor: 'rgba(182, 255, 0, 0.05)', border: '1px solid rgba(182, 255, 0, 0.2)' }}>
            <h3 className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#B6FF00' }}>
              BFS (Level Order) Traversal Result
            </h3>
            <div className="flex gap-2 flex-wrap">
              {traversalResult.map((val, idx) => (
                <div
                  ...
                  className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm transition-all duration-300 ${
                    idx === currentStep
                      ? 'bg-[#B6FF00] text-[#476700] scale-110 shadow-lg shadow-[#B6FF00]/50'
                      : 'bg-[#68fcbf] text-[#005e40]'
                  }`}
                >
                  {val}
                </div>
              ...
              {traversalResult.length === 0 && (
                <div className="text-sm text-gray-400 italic">Click "Start BFS" to begin level order traversal...</div>
              )}
            </div>
          </div>

          {/* Level Visualization */}
          {currentLevel > 0 && isTraversalPlaying && (
            <div className="mt-2 p-2 rounded-lg text-center" style={{ backgroundColor: 'rgba(255, 189, 46, 0.1)' }}>
              <span className="text-xs text-[#ffbd2e]">Currently traversing Level {currentLevel}</span>
            </div>
          )}

          {/* Info Cards */}
          <div className="grid grid-cols-3 gap-4 mt-4">
            <div className="p-3 rounded-lg" style={{ backgroundColor: 'rgba(182, 255, 0, 0.05)', border: '1px solid rgba(182, 255, 0, 0.1)' }}>
              <span className="text-[10px] font-bold uppercase text-gray-400">Time Complexity</span>
              <div className="text-xl font-bold text-[#B6FF00] mt-1">O(n)</div>
            </div>
            <div className="p-3 rounded-lg" style={{ backgroundColor: 'rgba(182, 255, 0, 0.05)', border: '1px solid rgba(182, 255, 0, 0.1)' }}>
              <span className="text-[10px] font-bold uppercase text-gray-400">Space Complexity</span>
              <div className="text-xl font-bold text-[#68fcbf] mt-1">O(w)</div>
              <div className="text-[8px] text-gray-500">w = max width</div>
            </div>
            <div className="p-3 rounded-lg" style={{ backgroundColor: 'rgba(182, 255, 0, 0.05)', border: '1px solid rgba(182, 255, 0, 0.1)' }}>
              <span className="text-[10px] font-bold uppercase text-gray-400">Data Structure</span>
              <div className="text-sm font-bold text-white mt-1">Queue (FIFO)</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {isPopupOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setIsPopupOpen(false)}>
          <div 
            ...
            style={{ backgroundColor: '#0a0f0d', border: '2px solid rgba(182, 255, 0, 0.3)' }}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-[rgba(182,255,0,0.2)]">
              <div>
                <h2 className="text-xl font-bold" style={{ color: '#B6FF00', fontFamily: 'Space Grotesk' }}>Binary Search Tree - BFS Traversal</h2>
                <p className="text-xs text-gray-400 mt-1">Level Order Traversal using Queue</p>
              </div>
              <button
                onClick={() => setIsPopupOpen(false)}
                className="p-2 rounded-lg hover:bg-[rgba(182,255,0,0.1)] transition-colors"
              >
            ...
            <div className="mt-4 pt-3 border-t border-[rgba(182,255,0,0.2)]">
              <div className="flex gap-2 flex-wrap">
                {traversalResult.map((val, idx) => (
                  <div
                    key={idx}
