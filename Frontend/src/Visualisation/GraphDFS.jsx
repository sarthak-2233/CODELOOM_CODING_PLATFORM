...
const GraphDFS = () => {
  ...
  const buildTreeStepByStep = useCallback(async () => {
    ...
    setLastAction('BST construction complete! Ready for DFS traversal');
    setIsBuilding(false);
  }, [arrayValues, canvasWidth, insertIntoBST]);

  // DFS Traversal (Inorder style - Left, Root, Right)
  const dfsTraversalAnimated = useCallback(async (node, result = [], depth = 0, onProgress) => {
    ...
    setStackState([node.value, ...(node.left ? [node.left.value] : []), ...(node.right ? [node.right.value] : [])]);
    
    await dfsTraversalAnimated(node.left, result, depth + 1, onProgress);
    ...
    onProgress(node, 'added', result.length - 1, depth);
    await new Promise(resolve => setTimeout(resolve, traversalSpeed));
    
    if (stopTraversalRef.current) return result;
    
    await dfsTraversalAnimated(node.right, result, depth + 1, onProgress);
  ...
  const startTraversal = useCallback(async () => {
    ...
    setLastAction(`Starting DFS (Inorder Traversal)...`);
    
    const result = [];
    
    await dfsTraversalAnimated(root, result, 0, (node, action, index, depth) => {
      if (action === 'added') {
        setTraversalResult([...result]);
        setCurrentStep(index);
        setLastAction(`Visited node: ${node.value} at depth ${depth}`);
    ...
    if (!stopTraversalRef.current) {
      setLastAction('DFS (Inorder) Traversal Complete!');
      setIsTraversalPlaying(false);
  ...
  }, [root, dfsTraversalAnimated, traversalSpeed]);
  ...
  const buildTreeInstant = useCallback(() => {
    ...
    setStackState([]);
    setCurrentDepth(0);
    setLastAction('Tree built instantly! Click Start DFS Traversal.');
  ...
  // Draw tree on canvas
  const drawTree = useCallback((ctx, node, depth = 0) => {
    if (!node) return;
    
    const isCurrentDepth = currentDepth === depth;
    ...
    if (node.left) {
      ...
      ctx.lineWidth = 2;
      ctx.stroke();
      drawTree(ctx, node.left, depth + 1);
    }
    ...
    if (node.right) {
      ...
      ctx.lineWidth = 2;
      ctx.stroke();
      drawTree(ctx, node.right, depth + 1);
    }
    ...
    } else if (isCurrentDepth && isTraversalPlaying) {
    ...
    ctx.strokeStyle = isHighlighted ? '#68fcbf' : 'rgba(104, 252, 191, 0.5)';
    ctx.lineWidth = 2;
    ctx.stroke();
    ...
    ctx.shadowBlur = 0;
  }, [highlightedNodeId, traversalResult, isTraversalPlaying, currentDepth, nodeRadius]);
    ...
    <div className="p-6 text-white h-full overflow-y-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm" style={{ color: '#E8FFF8', opacity: 0.7 }}>
          Inorder Traversal: Left → Root → Right (using Stack - Recursive)
        </p>
      </div>
            ...
            <div className="p-4 rounded-lg" style={{ backgroundColor: 'rgba(255, 189, 46, 0.05)', border: '1px solid rgba(255, 189, 46, 0.2)' }}>
              ...
              <div className="flex gap-1 flex-wrap">
                {stackState.map((val, idx) => (
                  <div key={idx} className="px-2 py-1 rounded text-xs bg-[#ffbd2e]/20 text-[#ffbd2e]">
                    {val}
                  </div>
                ))}
              </div>
        ...
        <div className="lg:col-span-3">
          ...
          <div className="mt-4 p-4 rounded-lg" style={{ backgroundColor: 'rgba(104, 252, 191, 0.05)', border: '1px solid rgba(104, 252, 191, 0.2)' }}>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {traversalResult.length === 0 && (
                <div className="text-sm text-gray-400 italic">Click "Start DFS" to begin depth-first search...</div>
              )}
            </div>
          </div>

          {/* Depth Visualization */}
          {currentDepth > 0 && isTraversalPlaying && (
            <div className="mt-2 p-2 rounded-lg text-center" style={{ backgroundColor: 'rgba(255, 189, 46, 0.1)' }}>
              <span className="text-xs text-[#ffbd2e]">Currently at depth {currentDepth}</span>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-4 mt-4">
            ...
            <div className="p-3 rounded-lg" style={{ backgroundColor: 'rgba(104, 252, 191, 0.05)', border: '1px solid rgba(104, 252, 191, 0.1)' }}>
              ...
              <div className="text-[8px] text-gray-500">h = height of tree</div>
            </div>
            <div className="p-3 rounded-lg" style={{ backgroundColor: 'rgba(104, 252, 191, 0.05)', border: '1px solid rgba(104, 252, 191, 0.1)' }}>
              <span className="text-[10px] font-bold uppercase text-gray-400">Data Structure</span>
              <div className="text-sm font-bold text-white mt-1">Stack (Recursion)</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setIsPopupOpen(false)}>
          ...
          >
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-[rgba(104,252,191,0.2)]">
              <div>
                <h2 className="text-xl font-bold" style={{ color: '#68fcbf', fontFamily: 'Space Grotesk' }}>Binary Search Tree - DFS Traversal</h2>
                <p className="text-xs text-gray-400 mt-1">Depth-First Search (Inorder: Left → Root → Right)</p>
              </div>
              <button
