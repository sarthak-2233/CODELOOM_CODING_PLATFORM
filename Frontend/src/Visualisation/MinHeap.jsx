...
const MinHeap = () => {
  ...
  // Min Heap bubble up
  const bubbleUpMin = useCallback(async (heapArray, index, onProgress) => {
  ...
  const bubbleDownMin = useCallback(async (heapArray, index, onProgress) => {
    let currentIndex = index;
    ...
    while (true) {
      ...
      if (leftIndex < size && heapArray[leftIndex] < heapArray[smallestIndex]) {
        smallestIndex = leftIndex;
      }
      if (rightIndex < size && heapArray[rightIndex] < heapArray[smallestIndex]) {
        smallestIndex = rightIndex;
      }
      
      if (smallestIndex !== currentIndex) {
        [heapArray[currentIndex], heapArray[smallestIndex]] = [heapArray[smallestIndex], heapArray[currentIndex]];
        onProgress([...heapArray], smallestIndex, currentIndex);
        await new Promise(resolve => setTimeout(resolve, animationSpeed));
  ...
  const insertValue = useCallback(async () => {
    ...
    setLastAction(`Inserting ${value} into Min Heap...`);
    ...
    const finalHeap = await bubbleUpMin(newHeap, newHeap.length - 1, (updatedHeap, parentIdx, childIdx) => {
      setHeap([...updatedHeap]);
    ...
    setInputValue('');
    setLastAction(`Inserted ${value} successfully! Minimum is now ${finalHeap[0]}`);
    
    stopAnimationRef.current = false;
  }, [inputValue, heap, animationSpeed, bubbleUpMin]);
  ...
  const extractMin = useCallback(async () => {
    ...
    setLastAction(`Extracting minimum value ${minValue} from Min Heap...`);
    ...
    if (newHeap.length > 0) {
      const finalHeap = await bubbleDownMin(newHeap, 0, (updatedHeap, swapIdx, currentIdx) => {
        setHeap([...updatedHeap]);
    ...
    setHighlightedIndex(null);
    setLastAction(`Extracted ${minValue}! New minimum is ${newHeap[0] || 'none'}`);
    
    stopAnimationRef.current = false;
  }, [heap, animationSpeed, bubbleDownMin]);
  ...
  const buildHeapFromArray = useCallback(async (arr) => {
    ...
    const newHeap = [...arr];
    setHeap(newHeap);
    setLastAction(`Building Min Heap from array...`);
    
    for (let i = Math.floor(newHeap.length / 2) - 1; i >= 0; i--) {
      if (stopAnimationRef.current) break;
      setHighlightedIndex(i);
      await new Promise(resolve => setTimeout(resolve, animationSpeed / 2));
      await bubbleDownMin(newHeap, i, (updatedHeap, swapIdx, currentIdx) => {
        setHeap([...updatedHeap]);
        setHighlightedIndex(swapIdx);
      });
    }
    ...
    setHighlightedIndex(null);
    setLastAction(`Min Heap built successfully! Minimum is ${newHeap[0]}`);
    setIsBuilding(false);
    stopAnimationRef.current = false;
  }, [animationSpeed, bubbleDownMin]);
  ...
  const drawHeap = useCallback((ctx, heapArray, width, height) => {
    ...
    const drawNodes = (node, idx = 0) => {
      ...
      ctx.strokeStyle = isHighlighted || idx === 0 ? '#b5fe00' : 'rgba(182, 255, 0, 0.5)';
      ctx.lineWidth = 2.5;
      ctx.stroke();
          ...
          <div className="rounded-lg overflow-hidden" style={{ backgroundColor: 'rgba(104, 252, 191, 0.03)', border: '1px solid rgba(104, 252, 191, 0.2)' }}>
            <div className="p-3 border-b border-[rgba(104,252,191,0.1)]">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#68fcbf' }}>Min Heap Visualization</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setIsPopupOpen(false)}>
          ...
          >
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-[rgba(104,252,191,0.2)]">
              <div>
                <h2 className="text-xl font-bold" style={{ color: '#68fcbf', fontFamily: 'Space Grotesk' }}>Min Heap Visualizer</h2>
                <p className="text-xs text-gray-400 mt-1">Parent ≤ Children (Smallest at root)</p>
              </div>
              <button
