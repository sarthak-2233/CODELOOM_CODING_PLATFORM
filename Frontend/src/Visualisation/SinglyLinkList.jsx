...
class SinglyLinkedList {
  ...
  // For animation: get node at index
  getNodeAt(index) {
    let current = this.head;
    let i = 0;
...
// ========== SINGLY LINKED LIST VISUALIZER COMPONENT ==========
const SinglyLinkedListVisualizer = () => {
  ...
  const currentTraversalIndex = useRef(0);
  const [traversalNode, setTraversalNode] = useState(null);
  ...
  const stopAnimation = useCallback(() => {
    ...
    setTraversalNode(null);
    currentTraversalIndex.current = 0;
  }, []);
  ...
  const startTraversal = useCallback(() => {
    ...
    if (nodes.length === 0) {
      showTemporaryMessage("List is empty! Add nodes first.", true);
      return;
    }
    ...
    animationRef.current = setInterval(() => {
      currentTraversalIndex.current++;
      if (currentTraversalIndex.current >= nodes.length) {
        // Animation complete
        stopAnimation();
        setTraversalNode(null);
        showTemporaryMessage("Traversal complete! ✅", false);
      } else {
        setTraversalNode(currentTraversalIndex.current);
        setActiveIndex(currentTraversalIndex.current);
      }
    }, animationSpeed);
  }, [list, animationSpeed, stopAnimation]);
  ...
  }, [isPlaying, startTraversal, stopAnimation]);
  ...
  const handlePopFront = useCallback(() => {
    if (list.size === 0) {
      showTemporaryMessage("List is empty!", true);
      return;
    }
  ...
  const handlePopBack = useCallback(() => {
    ...
    setList(newList);
    setActiveIndex(Math.max(0, list.size - 2));
      ...
      <aside className="fixed left-0 top-16 bottom-0 w-64 border-r border-[#B6FF00]/10 bg-[#0A0F0D] flex flex-col justify-between py-6 z-40">
        <div>
          ...
          <nav className="space-y-1">
            ...
            {/* Linked List Active Section - Singly is active */}
            ...
            <div className="ml-10 space-y-1 mt-1 mb-2">
              <div className="text-[#B6FF00] py-2 px-4 text-xs font-bold border-l border-[#B6FF00]/30">Singly Linked List</div>
              <div className="text-[#E8FFF8]/40 py-2 px-4 text-xs hover:text-[#B6FF00] transition-colors cursor-pointer">Doubly Linked List</div>
            </div>
            <div className="text-[#E8FFF8]/40 py-3 px-6 hover:bg-[#B6FF00]/5 hover:text-[#B6FF00] flex items-center gap-3 transition-colors duration-200 cursor-pointer">
          ...
          <div className="relative bg-[rgba(21,27,24,0.6)] backdrop-blur-[20px] rounded-lg p-8 h-[500px] border border-[#B6FF00]/5 overflow-hidden flex flex-col">
            <div className="flex justify-between items-center mb-12">
              <div>
                ...
                <p className="text-on-surface-variant text-sm">Active Structure: SinglyLinkedList()</p>
                {isTraversing && (
                  <p className="text-[#B6FF00] text-xs mt-1 animate-pulse">🔍 Traversing list...</p>
                )}
              </div>
            ...
            {/* Play/Stop Controls */}
            <div className="absolute top-24 right-8 flex items-center gap-3 bg-surface-container-high/80 backdrop-blur-sm rounded-full px-3 py-2 border border-[#B6FF00]/20 z-10">
              <button
                onClick={togglePlayPause}
                className={`px-4 py-1.5 rounded-full text-sm font-bold flex items-center gap-2 transition-all ${
                  isPlaying 
                    ? 'bg-error/20 text-error border border-error/30' 
                    : 'bg-[#B6FF00]/20 text-[#B6FF00] border border-[#B6FF00]/30 hover:bg-[#B6FF00]/30'
                }`}
              >
                <span className="material-symbols-outlined text-base">
                  {isPlaying ? 'stop' : 'play_arrow'}
                </span>
                {isPlaying ? 'Stop' : 'Play Traversal'}
              </button>
              <div className="w-px h-6 bg-[#B6FF00]/20"></div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-on-surface-variant">Speed:</span>
                <select
                  value={animationSpeed}
                  onChange={(e) => setAnimationSpeed(Number(e.target.value))}
                  className="bg-surface-container border border-[#B6FF00]/20 rounded-full px-2 py-1 text-xs text-on-surface font-mono focus:outline-none focus:border-[#B6FF00]"
                >
            ...
            <div className="flex-1 flex items-center justify-center gap-x-16 flex-wrap">
              {nodesArray.map((value, idx) => (
                <div key={`${idx}-${value}`} className="relative flex items-center">
                  <div className={`
                    ...
                    ${traversalNode === idx 
                      ? 'bg-[#B6FF00]/20 border-2 border-[#B6FF00] scale-110 shadow-[0_0_25px_rgba(182,255,0,0.5)] animate-pulse' 
                      : idx === activeIndex && !isTraversing
                        ? 'bg-[#B6FF00]/10 border-2 border-[#B6FF00] scale-105 shadow-[0_0_20px_rgba(182,255,0,0.3)]'
                        : 'bg-surface-container-high border border-[#B6FF00]/20 shadow-[0_0_15px_rgba(182,255,0,0.15)]'
                    }
                  `}>
                    <span className={`text-xs mb-1 font-label ${traversalNode === idx ? 'text-[#B6FF00] font-bold' : idx === activeIndex ? 'text-[#B6FF00]' : 'text-on-surface-variant'}`}>
                      {idx === 0 ? 'HEAD' : idx === totalNodes - 1 ? 'TAIL' : traversalNode === idx ? '🔍 VISITING' : `NODE`}
                    </span>
                    <span className={`text-2xl font-headline font-bold ${traversalNode === idx ? 'text-[#B6FF00] text-3xl' : idx === 0 ? 'text-[#B6FF00]' : 'text-on-surface'}`}>
                      {value}
                    </span>
                  </div>
                  {idx < totalNodes - 1 && (
                  ...
                  {idx === totalNodes - 1 && (
                    <div className="absolute -right-14 top-1/2 -translate-y-1/2 text-on-surface-variant opacity-30">
            ...
            {/* Toast Message */}
            {message && (
              <div className={`absolute bottom-6 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded-full text-sm font-bold animate-pulse shadow-lg z-20 ${
                message.isError ? 'bg-error/90 text-white' : 'bg-[#B6FF00] text-[#476700]'
              }`}>
                {message.text}
              </div>
            )}
          ...
          <div className="grid grid-cols-2 gap-8">
            ...
            <div className="bg-[rgba(21,27,24,0.6)] backdrop-blur-[20px] border border-[#B6FF00]/5 p-6 rounded-lg">
              ...
              <div className="flex flex-col justify-center h-full pb-4">
                ...
                <p className="text-xs text-on-surface-variant leading-relaxed">Each node requires memory for data and a next pointer.</p>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="bg-[rgba(21,27,24,0.6)] backdrop-blur-[20px] border border-[#B6FF00]/5 rounded-lg flex-1 flex flex-col overflow-hidden">
            <div className="bg-surface-container px-6 py-4 border-b border-[#B6FF00]/10 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-4 text-xs font-bold text-on-surface-variant tracking-wider uppercase">python_impl_singly.py</span>
              </div>
            ...
            <div className="p-6 font-mono text-sm leading-relaxed overflow-y-auto max-h-[500px]">
...
<span class="text-secondary">class</span> <span class="text-[#B6FF00]">SinglyLinkedList</span>:
    ...
    <span class="text-secondary">def</span> <span class="text-primary-dim">prepend</span>(<span class="text-primary">self</span>, value):
        new_node = Node(value)
        <span class="text-secondary">if not</span> <span class="text-primary">self</span>.head:
            <span class="text-primary">self</span>.head = new_node
            <span class="text-primary">self</span>.tail = new_node
        <span class="text-secondary">else</span>:
            new_node.next = <span class="text-primary">self</span>.head
            <span class="text-primary">self</span>.head = new_node

    <span class="text-secondary">def</span> <span class="text-primary-dim">pop_front</span>(<span class="text-primary">self</span>):
        <span class="text-secondary">if not</span> <span class="text-primary">self</span>.head: <span class="text-secondary">return None</span>
        val = <span class="text-primary">self</span>.head.value
        <span class="text-primary">self</span>.head = <span class="text-primary">self</span>.head.next
        <span class="text-secondary">if not</span> <span class="text-primary">self</span>.head:
            <span class="text-primary">self</span>.tail = <span class="text-secondary">None</span>
        <span class="text-secondary">return</span> val

    <span class="text-secondary">def</span> <span class="text-primary-dim">pop_back</span>(<span class="text-primary">self</span>):
        <span class="text-secondary">if not</span> <span class="text-primary">self</span>.head: <span class="text-secondary">return None</span>
        <span class="text-secondary">if</span> <span class="text-primary">self</span>.head == <span class="text-primary">self</span>.tail:
            val = <span class="text-primary">self</span>.head.value
            <span class="text-primary">self</span>.head = <span class="text-primary">self</span>.tail = <span class="text-secondary">None</span>
            <span class="text-secondary">return</span> val
        current = <span class="text-primary">self</span>.head
        <span class="text-secondary">while</span> current.next != <span class="text-primary">self</span>.tail:
            current = current.next
        val = <span class="text-primary">self</span>.tail.value
        <span class="text-primary">self</span>.tail = current
        <span class="text-primary">self</span>.tail.next = <span class="text-secondary">None</span>
        <span class="text-secondary">return</span> val

    <span class="text-secondary">def</span> <span class="text-primary-dim">traverse</span>(<span class="text-primary">self</span>):
        result = []
        current = <span class="text-primary">self</span>.head
        <span class="text-secondary">while</span> current:
            result.append(current.value)
            current = current.next
        <span class="text-secondary">return</span> result`}
              </pre>
            </div>
      ...
      <style>{`
        ...
        .bg-surface-container-high { background-color: #1a211e; }
        ...
        .bg-background { background-color: #0a0f0d; }
      `}</style>
    </div>
  );
};
