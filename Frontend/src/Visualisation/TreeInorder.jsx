...
const TreeInorder = () => {
  ...
  const inorderTraversalAnimated = useCallback(async (node, result = [], onProgress) => {
    ...
    await inorderTraversalAnimated(node.left, result, onProgress);
    ...
    await inorderTraversalAnimated(node.right, result, onProgress);
  ...
  const startTraversal = useCallback(async () => {
    ...
    setLastAction(`Starting Inorder Traversal (Left → Root → Right)...`);
    
    const result = [];
    
    await inorderTraversalAnimated(root, result, (node, action, index) => {
      if (action === 'added') {
    ...
    if (!stopTraversalRef.current && !pauseTraversalRef.current) {
      setLastAction('Inorder Traversal Complete!');
      setHighlightedNodeId(null);
      setIsTraversalPlaying(false);
      setIsPlaying(false);
  ...
  }, [root, inorderTraversalAnimated, traversalSpeed]);
  ...
  const drawTree = useCallback((ctx, node) => {
    ...
    ctx.strokeStyle = isHighlighted || isCurrent ? '#b5fe00' : 'rgba(182, 255, 0, 0.5)';
    ctx.lineWidth = 2;
    ctx.stroke();
    ...
    <div className="p-6 text-white h-full overflow-y-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm" style={{ color: '#E8FFF8', opacity: 0.7 }}>
          Left → Root → Right (produces sorted order in BST)
        </p>
      </div>
        ...
        <div className="lg:col-span-3">
          ...
          <div className="mt-4 p-4 rounded-lg" style={{ backgroundColor: 'rgba(182, 255, 0, 0.05)', border: '1px solid rgba(182, 255, 0, 0.2)' }}>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {traversalResult.length === 0 && (
                <div className="text-sm text-gray-400 italic">Click "Start" to begin inorder traversal...</div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mt-4">
            ...
            <div className="p-3 rounded-lg" style={{ backgroundColor: 'rgba(182, 255, 0, 0.05)', border: '1px solid rgba(182, 255, 0, 0.1)' }}>
              ...
              <div className="text-xl font-bold text-[#68fcbf] mt-1">O(h)</div>
            </div>
            <div className="p-3 rounded-lg" style={{ backgroundColor: 'rgba(182, 255, 0, 0.05)', border: '1px solid rgba(182, 255, 0, 0.1)' }}>
              <span className="text-[10px] font-bold uppercase text-gray-400">Pattern</span>
              <div className="text-sm font-bold text-white mt-1">Left → Root → Right</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setIsPopupOpen(false)}>
          ...
          >
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-[rgba(182,255,0,0.2)]">
              <div>
                <h2 className="text-xl font-bold" style={{ color: '#B6FF00', fontFamily: 'Space Grotesk' }}>Binary Search Tree - Inorder Traversal</h2>
                <p className="text-xs text-gray-400 mt-1">Left → Root → Right (Sorted Order)</p>
              </div>
              <button
