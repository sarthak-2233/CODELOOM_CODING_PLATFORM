...
const TreePostorder = () => {
  ...
  // Postorder traversal with animation (Left → Right → Root)
  const postorderTraversalAnimated = useCallback(async (node, result = [], onProgress) => {
    ...
    await postorderTraversalAnimated(node.left, result, onProgress);
    
    if (stopTraversalRef.current) return result;
    
    await postorderTraversalAnimated(node.right, result, onProgress);
  ...
  const startTraversal = useCallback(async () => {
    ...
    setLastAction(`Starting Postorder Traversal (Left → Right → Root)...`);
    
    const result = [];
    
    await postorderTraversalAnimated(root, result, (node, action, index) => {
      if (action === 'added') {
    ...
    if (!stopTraversalRef.current && !pauseTraversalRef.current) {
      setLastAction('Postorder Traversal Complete!');
      setHighlightedNodeId(null);
      setIsTraversalPlaying(false);
      setIsPlaying(false);
  ...
  }, [root, postorderTraversalAnimated, traversalSpeed]);
  ...
  const drawTree = useCallback((ctx, node) => {
    ...
    ctx.strokeStyle = isHighlighted || isCurrent ? '#ffbd2e' : 'rgba(255, 189, 46, 0.5)';
    ctx.lineWidth = 2;
    ctx.stroke();
    ...
    if (root) {
      ...
      const drawPopupTree = (node) => {
        ...
        if (node.left) {
          ...
          ctx.strokeStyle = 'rgba(255, 189, 46, 0.5)';
          ctx.lineWidth = 2.5;
          ctx.stroke();
        ...
        ctx.strokeStyle = isHighlighted ? '#ffbd2e' : 'rgba(255, 189, 46, 0.6)';
        ctx.lineWidth = 2.5;
        ctx.stroke();
  ...
  return (
    <div className="p-6 text-white h-full overflow-y-auto">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-2" style={{ color: '#ffbd2e', fontFamily: 'Space Grotesk' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="p-4 rounded-lg" style={{ backgroundColor: 'rgba(255, 189, 46, 0.05)', border: '1px solid rgba(255, 189, 46, 0.2)' }}>
            <h3 className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#ffbd2e' }}>
              Array Input
            </h3>
            <input
              type="text"
              ...
              className="w-full bg-[#1a211e] border border-[rgba(255,189,46,0.3)] rounded-lg px-3 py-2 text-sm text-white placeholder:text-gray-500 focus:outline-none focus:border-[#ffbd2e] transition-all mb-2"
            />
            <div className="flex gap-2 mb-2">
              <button
                onClick={applyCustomArray}
                className="flex-1 px-3 py-1.5 rounded-lg bg-[#ffbd2e] text-[#476700] text-xs font-bold hover:scale-105 transition-all"
              >
                Apply
              </button>
              <button
                onClick={useDefaultArray}
                className="flex-1 px-3 py-1.5 rounded-lg border border-[rgba(255,189,46,0.3)] text-xs hover:border-[#ffbd2e] hover:text-[#ffbd2e] transition-all"
              >
                Default
              </button>
            </div>
          ...
          <div className="p-4 rounded-lg" style={{ backgroundColor: 'rgba(255, 189, 46, 0.05)', border: '1px solid rgba(255, 189, 46, 0.2)' }}>
            ...
            <div className="space-y-2">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-[#ffbd2e] text-[#476700] text-sm font-bold hover:scale-105 transition-all disabled:opacity-50"
              >
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-[#1a211e] border border-[rgba(255,189,46,0.3)] text-sm hover:border-[#ffbd2e] transition-all disabled:opacity-50"
              >
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                ...
                <input
                  ...
                  className="flex-1"
                />
                <span className="text-xs text-[#ffbd2e] font-mono">{traversalSpeed}ms</span>
              </div>
              ...
              <div className="flex gap-2">
                {!isTraversalPlaying && !isPaused ? (
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-[#ffbd2e] text-[#476700] text-sm font-bold hover:scale-105 transition-all disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-[#ff7351] text-white text-sm font-bold hover:scale-105 transition-all"
                  >
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-[#ffbd2e]/10 border-2 border-[#ffbd2e] text-[#ffbd2e] text-sm font-bold hover:bg-[#ffbd2e]/20 transition-all"
          >
        ...
        <div className="lg:col-span-3">
          <div className="rounded-lg overflow-hidden" style={{ backgroundColor: 'rgba(255, 189, 46, 0.03)', border: '1px solid rgba(255, 189, 46, 0.2)' }}>
            <div className="p-3 border-b border-[rgba(255,189,46,0.1)]">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#ffbd2e' }}>Tree Visualization</span>
          ...
          {/* Traversal Result */}
          <div className="mt-4 p-4 rounded-lg" style={{ backgroundColor: 'rgba(255, 189, 46, 0.05)', border: '1px solid rgba(255, 189, 46, 0.2)' }}>
            <h3 className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#ffbd2e' }}>
            ...
            <div className="flex gap-2 flex-wrap">
              {traversalResult.map((val, idx) => (
                <div
                  ...
                  className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm transition-all duration-300 ${
                    idx === currentStep
                      ? 'bg-[#ffbd2e] text-[#476700] scale-110 shadow-lg shadow-[#ffbd2e]/50'
              ...
              {traversalResult.length === 0 && (
                <div className="text-sm text-gray-400 italic">Click "Start" to begin postorder traversal...</div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mt-4">
            <div className="p-3 rounded-lg" style={{ backgroundColor: 'rgba(255, 189, 46, 0.05)', border: '1px solid rgba(255, 189, 46, 0.1)' }}>
              <span className="text-[10px] font-bold uppercase text-gray-400">Time Complexity</span>
              <div className="text-xl font-bold text-[#ffbd2e] mt-1">O(n)</div>
            </div>
            <div className="p-3 rounded-lg" style={{ backgroundColor: 'rgba(255, 189, 46, 0.05)', border: '1px solid rgba(255, 189, 46, 0.1)' }}>
            ...
            <div className="p-3 rounded-lg" style={{ backgroundColor: 'rgba(255, 189, 46, 0.05)', border: '1px solid rgba(255, 189, 46, 0.1)' }}>
              ...
              <div className="text-sm font-bold text-white mt-1">Left → Right → Root</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {isPopupOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setIsPopupOpen(false)}>
          <div 
            ...
            style={{ backgroundColor: '#0a0f0d', border: '2px solid rgba(255, 189, 46, 0.3)' }}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-[rgba(255,189,46,0.2)]">
              <div>
                <h2 className="text-xl font-bold" style={{ color: '#ffbd2e', fontFamily: 'Space Grotesk' }}>Binary Search Tree - Postorder Traversal</h2>
                <p className="text-xs text-gray-400 mt-1">Left → Right → Root (Tree Deletion)</p>
              </div>
              <button
                onClick={() => setIsPopupOpen(false)}
                className="p-2 rounded-lg hover:bg-[rgba(255,189,46,0.1)] transition-colors"
              >
            ...
            <div className="mt-4 pt-3 border-t border-[rgba(255,189,46,0.2)]">
              <div className="flex gap-2 flex-wrap">
                {traversalResult.map((val, idx) => (
                  <div
                    key={idx}
