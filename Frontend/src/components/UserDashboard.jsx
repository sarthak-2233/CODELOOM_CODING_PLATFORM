...
const StatCard = ({ icon, label, value, accent, delay }) => (
  <div style={{
    ...
    <div>
      <div style={{ fontSize: 26, fontWeight: 800, color: '#F9FDF9', lineHeight: 1, fontFamily: "'Space Grotesk', monospace" }}>
        {value}
      </div>
      <div style={{ fontSize: 12, color: '#6B7570', marginTop: 4, textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 500 }}>
        {label}
      </div>
    </div>
    {/* glow */}
...
const DonutChart = ({ easy, medium, hard, total }) => {
  const size = 140;
  const cx = size / 2, cy = size / 2, r = 52, stroke = 12;
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ transform: 'rotate(-90deg)' }}>
        ...
        <text x={cx} y={cy} textAnchor="middle" dominantBaseline="middle"
          style={{ transform: 'rotate(90deg)', transformOrigin: `${cx}px ${cy}px`, fill: '#F9FDF9', fontSize: 22, fontWeight: 800, fontFamily: 'monospace' }}>
          {total}
        </text>
        <text x={cx} y={cy + 18} textAnchor="middle" dominantBaseline="middle"
          style={{ transform: 'rotate(90deg)', transformOrigin: `${cx}px ${cy}px`, fill: '#6B7570', fontSize: 10, fontFamily: 'monospace', letterSpacing: 1 }}>
          SOLVED
        </text>
      </svg>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {segments.map(seg => (
          <div key={seg.label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontSize: 12, color: '#A7ACA9', width: 52 }}>{seg.label}</span>
            <span style={{ fontSize: 14, fontWeight: 700, color: '#F9FDF9', fontFamily: 'monospace' }}>{seg.count}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ background: '#0A0F0D', color: '#F9FDF9', minHeight: '100vh', fontFamily: "'DM Sans', 'Space Grotesk', sans-serif" }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 14, color: '#B6FE00', fontWeight: 800,
          }}>
            {initials}
          </div>
          <button onClick={handleLogout} style={{
            ...
            color: '#FF7351', borderRadius: 8, padding: '7px 14px', cursor: 'pointer',
            fontSize: 13, fontWeight: 600,
          }}>
            <IconLogout /> Logout
          </button>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '36px 24px' }}>
        ...
        <div style={{
          ...
          borderRadius: 18, padding: '32px 36px', marginBottom: 32,
          ...
          <div style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 28, fontWeight: 900, color: '#B6FE00', flexShrink: 0,
          }}>
            {initials}
          </div>
          <div>
            <div style={{ fontSize: 28, fontWeight: 800, color: '#F9FDF9', letterSpacing: '-0.5px' }}>
              {firstName}
            </div>
            <div style={{ fontSize: 13, color: '#6B7570', marginTop: 4 }}>{emailId}</div>
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <span style={{ background: 'rgba(182,255,0,0.12)', border: '1px solid rgba(182,255,0,0.25)', color: '#B6FE00', borderRadius: 6, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>
                🏅 Active Coder
              </span>
              {totalSolved >= 50 && (
                <span style={{ background: 'rgba(104,252,191,0.12)', border: '1px solid rgba(104,252,191,0.25)', color: '#68FCBF', borderRadius: 6, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>
                  🔥 50+ Solved
                </span>
              )}
            </div>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: 0 }}>
          {[
            ...
            { id: 'problems', label: `Solved Problems (${totalSolved})` },
        ...
        {/* ── OVERVIEW TAB ── */}
        {activeTab === 'overview' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, animation: 'fadeUp 0.4s ease both' }}>

            {/* Difficulty Breakdown */}
            <div style={{ background: 'rgba(21,27,24,0.7)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '24px 28px' }}>
              <h3 style={{ margin: '0 0 20px', fontSize: 14, fontWeight: 700, color: '#A7ACA9', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                Difficulty Breakdown
              </h3>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {topTags.map(([tag, count]) => (
                    <div key={tag} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <span style={{ flex: 1, fontSize: 13, color: '#D4DDD8', textTransform: 'capitalize' }}>{tag}</span>
                      <div style={{ height: 4, width: 80, background: 'rgba(255,255,255,0.06)', borderRadius: 10, overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${(count / (topTags[0]?.[1] || 1)) * 100}%`, background: 'linear-gradient(90deg, #B6FE00, #68FCBF)', borderRadius: 10 }} />
                      </div>
                      <span style={{ fontSize: 12, fontWeight: 700, color: '#B6FE00', minWidth: 20, textAlign: 'right', fontFamily: 'monospace' }}>{count}</span>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <div style={{ gridColumn: '1 / -1', background: 'rgba(21,27,24,0.7)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '24px 28px' }}>
              <h3 style={{ margin: '0 0 20px', fontSize: 14, fontWeight: 700, color: '#A7ACA9', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                Recently Solved
              </h3>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  {solvedProblems.slice(0, 5).map((p, i) => {
                    ...
                    return (
                      ...
                      >
                        <span style={{ fontSize: 12, color: '#6B7570', minWidth: 24, fontFamily: 'monospace' }}>#{i + 1}</span>
                        <span style={{ flex: 1, fontSize: 14, color: '#D4DDD8', fontWeight: 500 }}>{p.title}</span>
                        <span style={{
                        ...
                        <span style={{ color: '#68FCBF', fontSize: 18 }}>✓</span>
                      </div>
                    );
                  })}
                </div>
        ...
        {activeTab === 'problems' && (
          <div style={{ animation: 'fadeUp 0.4s ease both' }}>
            ...
            {loadingSolved ? (
              ...
              <div style={{ background: 'rgba(21,27,24,0.6)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
                {/* Header */}
                <div style={{ display: 'grid', gridTemplateColumns: '40px 1fr 100px 120px', gap: 16, padding: '12px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', fontSize: 11, fontWeight: 700, color: '#6B7570', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                ...
                {filtered.map((p, i) => {
                  ...
                  return (
                    ...
                    >
                      <span style={{ fontSize: 12, color: '#6B7570', fontFamily: 'monospace' }}>{i + 1}</span>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ color: '#68FCBF', fontSize: 14 }}>✓</span>
                        <span style={{ fontSize: 14, color: '#D4DDD8', fontWeight: 500 }}>{p.title}</span>
                      </div>
                      <span style={{ padding: '3px 10px', borderRadius: 6, fontSize: 11, fontWeight: 700, background: dc.bg, color: dc.text, border: `1px solid ${dc.border}`, width: 'fit-content' }}>
                        {p.difficulty?.charAt(0).toUpperCase() + p.difficulty?.slice(1)}
                      </span>
                ...
                {topTags.map(([tag, count], i) => (
                  <div key={tag} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                      <span style={{ fontSize: 15, fontWeight: 700, color: '#D4DDD8', textTransform: 'capitalize' }}>{tag}</span>
                      <span style={{ fontSize: 22, fontWeight: 900, color: '#B6FE00', fontFamily: 'monospace' }}>{count}</span>
                    </div>
                    ...
                    <div style={{ marginTop: 8, fontSize: 11, color: '#6B7570' }}>
      ...
      {/* Glow blobs */}
      <div style={{ position: 'fixed', top: '-10%', right: '-10%', width: '45%', height: '45%', background: 'rgba(182,255,0,0.04)', filter: 'blur(120px)', borderRadius: '50%', zIndex: 0, pointerEvents: 'none' }} />
